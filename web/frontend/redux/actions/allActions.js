...
export const editVisibleFlag = createAsyncThunk(
  ...
  async (editCustomFormData, { rejectWithValue }) => {
    try {
      const response = await axios.put(
        `/custom_form/visible/${editCustomFormData._id}?appId=${editCustomFormData.appId}`,
...
export const loadMoreSubmission = createAsyncThunk(
  ...
  async (data) => {
    try {
      const response = await axios.get(
        `/submission/data/loadmore/${data.order}?appId=${data.appId}&page=${data.page}&per_page=${data.per_page}`
      );
      return response?.data;
    } catch (error) {
      return rejectWithValue(error.response.data);
...
export const deleteSubmissionData = createAsyncThunk(
  ...
  async (
    { submissionIdArr, filterFormId, path, query, appId },
    ...
    try {
      ...
      filterFormId
        ...
        : dispatch(sortNFilterSubmission({ path, query, appId }));
      toast.success(response?.data?.msg, toastConfig);
    } catch (error) {
      toast.error(error?.response?.msg, toastConfig);
      return rejectWithValue(error.response.data);

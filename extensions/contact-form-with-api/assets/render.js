...
const generateElementId = (type, id) => {
  ...
  catchFormDivAndAppendForm = async (data) => {
    !(function (e) {
    ...
    let recaptchadata;
    enableRecaptcha && (recaptchadata = await handleRecaptcha());
    ...
    const formElementId = generateElementId(
      ...
      submitButtonId = generateElementId(
        "submit",
        `submit-${data?.elementData?._id}`
      ),
      loaderElementId = generateElementId(
        "loader",
        `ipsFormPreviewLoader-${data?.elementData?._id}`
      ),
      bannerElementId = generateElementId(
        "banner",
        `polaris-banner-${data.elementData?._id}`
      ),
      checkElements = (plan, elem, checkPreview) => {
    ...
    const createSubmission = async (data, formId) => {
      ...
      if (data && Object.keys(data).length > 0) {
        for (const key in data) {
          ...
          else finalFormData.append(key, data[key]);
        }
      }
    ...
    function validateInput(fieldType, fieldValue, obj) {
      ...
      if (Array.isArray(value)) {
        if (value?.length === 0) return validation?.["required"];
      } else {
    ...
    const checkValidation = (type, value) => {
      formFieldData = formFieldData.map((item) => {
        if (item.fieldType === type && value) {
          let fieldData = { ...item, fieldValue: value },
            error = validateInput(fieldData.fieldType, fieldData.fieldValue);
      ...
      handleChange = (renderId, event, confirmPassword) => {
      ...
      setDefaultValue = () => {
        formElement.forEach((data) => {
          const { id, attributes, inputId } = data,
            initialErrorElement = document.getElementById(`${inputId}_error`),
            confirmPasswordError =
              attributes.confirmPassword &&
              document.getElementById(`${inputId}_CPError`),
            captchaError = document.getElementById("captcha_error");
          ...
          if (id === "radio") {
            ...
            for (let radio of redioElements) {
              if (attributes?.default_value === radio.value) radio.checked = true;
            }
          } else if (id === "checkbox") {
      ...
      handleSubmit = async (event, buttonText, formId) => {
      ...
      headerFieldData = data?.elementData?.customForm[1].header,
      formElementData = data?.elementData?.customForm[2].element,
      footerFieldData = data?.elementData?.customForm[4].footer,
      appearanceFields = handleAppereance(data?.appearanceData),
      validationFields = handleAppereance(data?.validationData),
      afterSubmitFields = handleAppereance(data?.afterSubmit),
      mainFormElement = data.element,
      user = data.user;
    ...
    let selectedBackground =
      ...
      footerAlign = footerFieldData?.attributes?.footerAlign,
      inputStyles = {},
      appearanceStyle = appearanceFields && appearanceFields?.appearanceStyle;

    if (appearanceStyle === "classicRounded") inputStyles.borderRadius = "20px";
...
function fetchData() {
  setTimeout(async () => {
    ...
    Array.from(mainElement).forEach(async (element) => {
